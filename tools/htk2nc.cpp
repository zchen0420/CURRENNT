...
void FloatCopy2DArray(float *src, float *dst, size_t m, size_t nsrc, size_t ndst, ptrdiff_t off)
{
    uint8_t *src_ = (uint8_t*) src;
    uint8_t *dst_ = (uint8_t*) dst;
    uint8_t *p_src = src_;
...
{
    std::string gitcommitstr = "undefined git commit information";
#ifdef GITCOMMIT
    gitcommitstr = GITCOMMIT;
    cout << gitcommitstr <<std::endl;
#endif
    if (argc < 3) {
    ...
    if (argc > 3) {
      ...
      cout <<"mappingDelim: '"<<mappingDelim<<"'"<<endl;
    }
    if (argc > 4) {
    ...
    while (!fs.eof()) {
	    ...
	    if (nInputsLocal <= 0 || nInputsLocal > tokens.size() - 1) {
              cerr << "Number of input HTK files (2nd column) is out of range! nInputsLocal " \
                  << nInputsLocal << " tokens.size(): " << tokens.size()<< endl;
              return -1;
        }else if(nInputsLocal == tokens.size() -1){
            isPredictmode = true;
        }
            if (first) {
...
/*            if (first) {
    ...
    if (isClassification) {
        ...
        if(isPredictmode){
            cout << "output size: undefined("<< output_size << ") is predict mode" << endl;
        }else{
...
//cerr << "f = "<<f<<" s= "<<s<<endl;
            ...
            FloatCopy2DArray(h.rawData, inp_buf, seqLens[s], vect_sizes[f], input_size, input_start);
            delete[] h.rawData;
...
//        }
        ...
        // regression outputs
            ...
            for (int f = nInputs; f < mapping[s].size(); ++f) {
                ...
                //cout << "target file: " << h.nSamples << " x " << h.sampleSize / 4 << endl;
                // (seqLen x vect_size) matrix to (seqLen x output_size) matrix at offset output_start
                FloatCopy2DArray(h.rawData, outp_buf, seqLens[s], vect_sizes[f], output_size, output_start);
                delete[] h.rawData;
