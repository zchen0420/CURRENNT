...
#ifndef LAYERS_NOIGLSTMLAYER_HPP
#define LAYERS_NOIGLSTMLAYER_HPP
...
namespace layers {
     ...
     * connections and one cell per block, but no input gate
     *
     * weights; with P = precedingLayer().size() and L = size():
     *    ~ weights from preceding layer:
     *        - [0 .. PL-1]:    net input
     *        - [1PL .. 2PL-1]: forget gate
     *        - [2PL .. 3PL-1]: output gate
     *    ~ bias weights:
     *        - [3PL + 0  .. 3PL + L-1]:  net input
     *        - [3PL + 1L .. 3PL + 2L-1]: forget gate
     *        - [3PL + 2L .. 3PL + 3L-1]: output gate
     *    ~ internal weights (from other cells in the same layer):
     *        - [3(P+1)L + 0   .. 3(P+1)L + LL-1]:  net input
     *        - [3(P+1)L + 1LL .. 3(P+1)L + 2LL-1]: forget gate
     *        - [3(P+1)L + 2LL .. 3(P+1)L + 3LL-1]: output gate
     *    ~ peephole weights (from cell state to all gates in the same cell):
     *        - [3(P+1+L)L + 0   .. 3(P+1+L)L + L-1]:  forget gate
     *        - [3(P+1+L)L + 1LL .. 3(P+1+L)L + 2L-1]: output gate
     *
     * @param TDevice The computation device (Cpu or Gpu)
     *********************************************************************************************/
    template <typename TDevice>
    class NoIgLstmLayer : public TrainableLayer<TDevice>
    {
...
#endif // LAYERS_NOIGLSTMLAYER_HPP
