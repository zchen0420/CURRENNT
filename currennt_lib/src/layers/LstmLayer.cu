...
namespace {
    ...
    struct ResortOutputsFn
    {
        ...
        __host__ __device__ real_t operator() (const int &outputIdx) const
        {
            ...
            if (valIdx < effLayerSize) {
                return fwOutputs[offset + outputLag * effLayerSize];
            } else if (patIdx < outputLag) {
                return bwOutputs[offset - effLayerSize + curMaxSeqEffSize];
            } else {
                return bwOutputs[offset - effLayerSize + outputLag * effLayerSize];
            }
        }
    };
    ...
    struct ComputeBlockErrorsFn
    {
        ...
        __host__ __device__ void operator() (const thrust::tuple<const real_t&, int, bool, bool, bool, bool, bool> &t) const
        {
            // unpack the tuple
            real_t outputErr    = t.get<0>();
            int    outputIdx    = t.get<1>();
            ...
            bool   withInput    = t.get<4>();
            bool   withOutput   = t.get<5>();
            bool   checkPatType = t.get<6>();
            ...
            if (withOutput) {
                ...
                ogDelta = gate_act_fn_t::deriv(ogAct) * cell_output_act_fn_t::fn(cellState) * outputErr;

                // calculate the cell state error
                cellStateErr = ogAct * cell_output_act_fn_t::deriv(cell_output_act_fn_t::fn(cellState)) * outputErr + ogPeepWeights[blockIdx] * ogDelta;
            }
            ...
            if (withInput) {
                ...
                niDelta = igAct * cell_input_act_fn_t::deriv(niAct) * cellStateErr;

                // calculate the input gate delta
                igDelta = gate_act_fn_t::deriv(igAct) * niAct * cellStateErr;
            }
...
namespace layers {
    ...
    LstmLayer<TDevice>::LstmLayer(const helpers::JsonValue &layerChild, 
        ...
        , m_outputLag            (helpers::safeJsonGetInt(layerChild, "lag"))
        , m_directOutput         (!m_isBidirectional && m_outputLag == 0)
    {
        if (m_isBidirectional && this->size() % 2 != 0)
            throw std::runtime_error("Cannot create a bidirectional layer with an odd layer size");
        if (m_outputLag < 0)
            throw std::runtime_error("Timestep Lag should be a non-negative int");
        ...
        for (int fwbwArrIdx = 0; fwbwArrIdx < (m_isBidirectional ? 2 : 1); ++fwbwArrIdx) {
            ...
            Cpu::real_vector tmp((this->outputs().size() + m_outputLag * this->parallelSequences() * els) / (m_isBidirectional ? 2 : 1), 0);
            ...
            if (m_directOutput) {
                ...
                fwbw->tmpOutputs      = tmp;
                fwbw->tmpOutputErrors = Cpu::real_vector((this->outputs().size()) / (m_isBidirectional ? 2 : 1), 0);
            }
            ...
            for (int timestep = 0; timestep < this->maxSeqLength() + m_outputLag; ++timestep) {
    ...
    template <typename TDevice>
    int LstmLayer<TDevice>::ouputLag() const
    {
        return m_outputLag;
    }
    ...
    void LstmLayer<TDevice>::loadSequences(const data_sets::DataSetFraction &fraction)
    {
        ...
        for (int fwbwArrIdx = 0; fwbwArrIdx < (m_isBidirectional ? 2 : 1); ++fwbwArrIdx) {
            ...
            int rows = this->size() / (m_isBidirectional ? 2 : 1);
            int cols = (this->curMaxSeqLength() + m_outputLag) * this->parallelSequences();
    ...
    void LstmLayer<TDevice>::computeForwardPass()
    {
        ...
        {{
            ...
            for (int timestep = 0; timestep < this->curMaxSeqLength() + m_outputLag; ++timestep) {
                ...
                if (timestep != 0) {
                    if (timestep < this->curMaxSeqLength()) {
                        ...
                        m_fw.timestepMatrices[timestep].ogActs.assignProduct(m_fw.weightMatrices.ogInternal, true, m_fw.timestepMatrices[timestep - 1].tmpOutputs, false);
                    }
                }
            ...
            if (m_isBidirectional) {
                ...
                bool firstlap = true;
                for (int timestep = this->curMaxSeqLength() - 1; timestep >= -m_outputLag; --timestep) {
                    const int timestepLap = (timestep < 0) ? timestep + this->curMaxSeqLength() + m_outputLag : timestep;
                    // collect outputs from previous timestep
                    if (timestep != this->curMaxSeqLength() - 1) {
                        if (timestep < 0) {
                            if (firstlap) {
                                firstlap = false;
                                m_bw.timestepMatrices[timestepLap].niActs.assignProduct(m_bw.weightMatrices.niInternal, true, m_bw.timestepMatrices[0].tmpOutputs, false);
                                m_bw.timestepMatrices[timestepLap].igActs.assignProduct(m_bw.weightMatrices.igInternal, true, m_bw.timestepMatrices[0].tmpOutputs, false);
                                m_bw.timestepMatrices[timestepLap].fgActs.assignProduct(m_bw.weightMatrices.fgInternal, true, m_bw.timestepMatrices[0].tmpOutputs, false);
                                m_bw.timestepMatrices[timestepLap].ogActs.assignProduct(m_bw.weightMatrices.ogInternal, true, m_bw.timestepMatrices[0].tmpOutputs, false);
                            } else {
                                m_bw.timestepMatrices[timestepLap].niActs.assignProduct(m_bw.weightMatrices.niInternal, true, m_bw.timestepMatrices[timestepLap + 1].tmpOutputs, false);
                                m_bw.timestepMatrices[timestepLap].igActs.assignProduct(m_bw.weightMatrices.igInternal, true, m_bw.timestepMatrices[timestepLap + 1].tmpOutputs, false);
                                m_bw.timestepMatrices[timestepLap].fgActs.assignProduct(m_bw.weightMatrices.fgInternal, true, m_bw.timestepMatrices[timestepLap + 1].tmpOutputs, false);
                                m_bw.timestepMatrices[timestepLap].ogActs.assignProduct(m_bw.weightMatrices.ogInternal, true, m_bw.timestepMatrices[timestepLap + 1].tmpOutputs, false);
                            }
                        } else {
                            m_bw.timestepMatrices[timestepLap].niActs.addProduct(m_bw.weightMatrices.niInternal, true, m_bw.timestepMatrices[timestepLap + 1].tmpOutputs, false);
                            m_bw.timestepMatrices[timestepLap].igActs.addProduct(m_bw.weightMatrices.igInternal, true, m_bw.timestepMatrices[timestepLap + 1].tmpOutputs, false);
                            m_bw.timestepMatrices[timestepLap].fgActs.addProduct(m_bw.weightMatrices.fgInternal, true, m_bw.timestepMatrices[timestepLap + 1].tmpOutputs, false);
                            m_bw.timestepMatrices[timestepLap].ogActs.addProduct(m_bw.weightMatrices.ogInternal, true, m_bw.timestepMatrices[timestepLap + 1].tmpOutputs, false);
                        }
                    }
                    ...
                    // compute outputs
                    thrust::transform(
                        thrust::counting_iterator<int>(n*timestepLap),
                        thrust::counting_iterator<int>(n*timestepLap) + n,
                        thrust::make_zip_iterator(
                            thrust::make_tuple(
        ...
        if (m_directOutput) {
        ...
        } else if (!m_isBidirectional){
            for (int i = 0; i < this->curMaxSeqLength() * this->parallelSequences(); i++) {
                const int tgtStart = i * this->size();
                const int srcStart = tgtStart + m_outputLag * this->size();
                thrust::copy_n( m_fw.tmpOutputs.begin() + srcStart, this->size(), this->_outputs().begin() + tgtStart);
            }
        } else {
            ...
            fn.outputLag        = m_outputLag;
            fn.curMaxSeqEffSize = this->curMaxSeqLength() * this->size() / 2;
    ...
    void LstmLayer<TDevice>::computeBackwardPass()
    {
        ...
        {{
            ...
            for (int timestep = this->curMaxSeqLength()-1; timestep >= -m_outputLag; --timestep) {
                const int timestepLag = timestep+m_outputLag;
                // collect errors from previous timestep
                if (timestep != this->curMaxSeqLength()-1) {
                    if (timestep >= 0) {
                        m_fw.timestepMatrices[timestepLag].tmpOutputErrors.addProduct(m_fw.weightMatrices.ogInternal, false, m_fw.timestepMatrices[timestep+1].ogDeltas, false);
                    }
                    if (timestep < this->curMaxSeqLength() - m_outputLag) {
                        m_fw.timestepMatrices[timestepLag].tmpOutputErrors.addProduct(m_fw.weightMatrices.niInternal, false, m_fw.timestepMatrices[timestep+1].niDeltas, false);
                        m_fw.timestepMatrices[timestepLag].tmpOutputErrors.addProduct(m_fw.weightMatrices.igInternal, false, m_fw.timestepMatrices[timestep+1].igDeltas, false);
                    }
                        m_fw.timestepMatrices[timestepLag].tmpOutputErrors.addProduct(m_fw.weightMatrices.fgInternal, false, m_fw.timestepMatrices[timestep+1].fgDeltas, false);
                }
                ...
                thrust::for_each(
                    thrust::make_zip_iterator(
                        thrust::make_tuple(
                            m_fw.tmpOutputErrors.begin() + n*timestepLag,
                            thrust::counting_iterator<int>(n*timestep),
                            thrust::constant_iterator<bool>(timestep == this->curMaxSeqLength()-1),
                            thrust::constant_iterator<bool>(!timestep),
                            thrust::constant_iterator<bool>(timestepLag < this->curMaxSeqLength()),
                            thrust::constant_iterator<bool>(timestep >= 0 ),
                            thrust::constant_iterator<bool>(timestep >= this->curMinSeqLength()))),
                    thrust::make_zip_iterator(
                        thrust::make_tuple(
                            m_fw.tmpOutputErrors.begin() + n*timestepLag+n,
                            thrust::counting_iterator<int>(n*timestep)+n,
                            thrust::constant_iterator<bool>(timestep == this->curMaxSeqLength()-1)+n,
                            thrust::constant_iterator<bool>(!timestep)+n,
                            thrust::constant_iterator<bool>(timestepLag < this->curMaxSeqLength())+n,
                            thrust::constant_iterator<bool>(timestep >= 0 )+n,
                ...
                for (int timestep = 0; timestep < this->curMaxSeqLength()+m_outputLag; ++timestep) {
                    ...
                    thrust::for_each(
                        thrust::make_zip_iterator(
                            ...
                            thrust::constant_iterator<bool>(!timestep),
                            thrust::constant_iterator<bool>(timestep == this->curMaxSeqLength()-1),
                            thrust::constant_iterator<bool>(timestep < this->curMaxSeqLength()),
                            thrust::constant_iterator<bool>(timestep >= m_outputLag ),
                            thrust::constant_iterator<bool>(timestep >= this->curMinSeqLength()))),
                        thrust::make_zip_iterator(
                            thrust::make_tuple(m_bw.tmpOutputErrors.begin() + n*timestep+n,
                                ...
                                thrust::constant_iterator<bool>(!timestep)+n,
                                thrust::constant_iterator<bool>(timestep == this->curMaxSeqLength()-1)+n,
                                thrust::constant_iterator<bool>(timestep < this->curMaxSeqLength())+n,
                                thrust::constant_iterator<bool>(timestep >= m_outputLag )+n,
                                thrust::constant_iterator<bool>(timestep >= this->curMinSeqLength())+n)),
                        fn
                        );
                }
        ...
        {{
            ...
            fn.fwOutputs             = helpers::getRawPointer(m_fw.tmpOutputs);// !!!err
            fn.bwOutputs             = helpers::getRawPointer(m_bw.tmpOutputs);
