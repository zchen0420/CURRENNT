...
#include "RelativeSsePostOutputLayer.hpp"
...
namespace {
    ...
    struct ComputeSseFn
    {
        ...
        __host__ __device__ real_t operator() (const thrust::tuple<real_t, real_t, int> &values) const
        {
            ...
            // calculate the error
            if(target == 0.0f){
                printf("act %f, tar %f", output, target);
            }
            real_t diff = (target - output) / target;
            return (diff * diff);
        }
    };
    ...
    struct ComputeOutputErrorFn
    {
        ...
        __host__ __device__ real_t operator() (const thrust::tuple<const real_t&, const real_t&, int> &t) const
        {
            ...
            // calculate the error
            if(targetOutput == 0.0f){
                printf("act %f, tar %f", actualOutput, targetOutput);
            }
            real_t error = (actualOutput - targetOutput)/targetOutput;
...
namespace layers {
    ...
    RelativeSsePostOutputLayer<TDevice>::RelativeSsePostOutputLayer(const helpers::JsonValue &layerChild, Layer<TDevice> &precedingLayer)
        : PostOutputLayer<TDevice>(layerChild, precedingLayer, precedingLayer.size())
    {
    }

    template <typename TDevice>
    RelativeSsePostOutputLayer<TDevice>::~RelativeSsePostOutputLayer()
    {
    }

    template <typename TDevice>
    const std::string& RelativeSsePostOutputLayer<TDevice>::type() const
    {
        static const std::string s("rsse");
        return s;
    }

    template <typename TDevice>
    real_t RelativeSsePostOutputLayer<TDevice>::calculateError()
    {
    ...
    template <typename TDevice>
    void RelativeSsePostOutputLayer<TDevice>::computeForwardPass()
    {
    }

    template <typename TDevice>
    void RelativeSsePostOutputLayer<TDevice>::computeBackwardPass()
    {
    ...
    template class RelativeSsePostOutputLayer<Cpu>;
    template class RelativeSsePostOutputLayer<Gpu>;
