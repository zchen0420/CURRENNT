...
namespace internal {
    ...
    // readNcArrayHelper template for float, int and double
    template <typename T>
    ...
    // with template helper above, array can be read.
    template <typename T>
    ...
    // convertion from targetClasses to output v.
...
namespace data_sets {
    ...
    Cpu::real_vector DataSet::_loadInputMultiplyersFromCache(const sequence_t &seq)
    {
        Cpu::int_vector v(seq.length);

        m_cacheFile.seekg(seq.m2mmBegin);
        m_cacheFile.read((char*)v.data(), sizeof(int) * v.size());
        assert(m_cacheFile.tellg() - seq.m2mmBegin == v.size() * sizeof(int));
    ...
    {
        const int context_left = Configuration::instance().inputLeftContext();
        const int context_right = Configuration::instance().inputRightContext();
        const int context_length = context_left + context_right + 1;
        const int output_lag = Configuration::instance().outputTimeLag();
        const bool append_multiplyer_to_input = Configuration::instance().includingMultiplyers();
        ...
        frac->m_maxSeqLength += output_lag;
        frac->m_minSeqLength += output_lag;
        const int maxSeqLengthWithLag = frac->m_maxSeqLength;

        // allocate memory for the fraction
        frac->m_inputs  .resize(maxSeqLengthWithLag * m_parallelSequences * frac->m_inputPatternSize, 0);
        frac->m_patTypes.resize(maxSeqLengthWithLag * m_parallelSequences, PATTYPE_NONE);
        if (m_isM2mmSerialization) frac->m_inputMultiplyers.resize(maxSeqLengthWithLag * m_parallelSequences, 0);

        if (m_isClassificationData)
            frac->m_targetClasses.resize(maxSeqLengthWithLag * m_parallelSequences, -1);
        else
            frac->m_outputs.resize(maxSeqLengthWithLag * m_parallelSequences * m_outputPatternSize);
        ...
        for (int i = 0; i < m_parallelSequences; ++i) {
            ...
            for (int timestep = 0; timestep < seq.length; ++timestep) {
                ...
                for (int offset_in = -context_left; offset_in <= context_right; ++offset_in) {
                    ...
                    thrust::copy_n(inputs.begin() + srcStart, m_inputPatternSize, frac->m_inputs.begin() + tgtStart); // good logi and bad mem
                    ++offset_out;
                }
            }
            ...
            // record multiplyers
            if (m_isM2mmSerialization) {
                Cpu::real_vector inputMultiplyers = _loadInputMultiplyersFromCache(seq);
                for (int timestep = 0; timestep < seq.length; ++timestep) {
                    const int xyPos = timestep * m_parallelSequences + i;
                    frac->m_inputMultiplyers[xyPos] = inputMultiplyers[timestep];// still need to be normalized
                    if (append_multiplyer_to_input)
                        frac->m_inputs[xyPos + m_inputPatternSize - 1] = inputMultiplyers[timestep];
                }
            }
            ...
            if (m_isClassificationData) {
                ...
                for (int timestep = 0; timestep < seq.length; ++timestep) {
                    // int tgt = 0; // default class (make configurable? - no)
                    frac->m_targetClasses[(timestep + output_lag) * m_parallelSequences + i] = targetClasses[timestep];
                }
            }
            // outputs
            else {
                ...
                for (int timestep = 0; timestep < seq.length; ++timestep) {
                    ...
                    int tgtStart = m_outputPatternSize * ((timestep + output_lag) * m_parallelSequences + i);
                    thrust::copy_n(outputs.begin() + srcStart, m_outputPatternSize, frac->m_outputs.begin() + tgtStart);
                        ...
                        //     frac->m_outputs[tgtStart + oi] = 1.0f; // default value (make configurable? - still no)
                }
            }

            // pattern types
            for (int timestep = 0; timestep < seq.length + output_lag; ++timestep) {
        ...
        , m_curFirstSeqIdx   (-1)
    {
        int ret;
        int ncid;
        const bool isTrainingMode = Configuration::instance().trainingMode();
            ...
            nc_itr != ncfiles.end(); ++nc_itr)
        {
            ...
            try {
                ...
                if (first_file) {
                    ...
                    m_isM2mmSerialization  = internal::hasNcDimension (ncid, "recordMultiplyer");
                    if (m_isM2mmSerialization && Configuration::instance().includingMultiplyers()) {
                        if (Configuration::instance().outputTimeLag() != 0)
                            std::runtime_error("dynamic time lag for multiplyers is not implemented");
                        m_inputPatternSize++;
                        std::cerr << "This .nc data is capable with input multiplying, extending input size to "
                            << m_inputPatternSize << std::endl;
                    }
                    ...
                    if (m_isM2mmSerialization != internal::hasNcDimension (ncid, "recordMultiplyer"))
                        throw std::runtime_error("Field recordMultiplyers mismatch in NC files");
                }
                ...
                for (std::vector<sequence_t>::iterator seq = sequences.begin(); seq != sequences.end(); ++seq) {
                    ...
                    // read recordMultiplyers and store them in the cache file if they exist
                    if ( m_isM2mmSerialization ) {
                        seq->m2mmBegin = m_cacheFile.tellp();
                            Cpu::int_vector recordMultiplers = internal::readNcArray<int>(ncid, "recordMultiplyers", m2mmBegin, seq->length);
                            m_cacheFile.write((const char*)recordMultiplers.data(), sizeof(int) * recordMultiplers.size());
                            assert (m_cacheFile.tellp() - seq->m2mmBegin == seq->length * sizeof(int));
                    }

                    inputsBegin  += seq->length;
                    targetsBegin += seq->length;
                    m2mmBegin    += seq->length;
                }
                ...
                if (first_file) {
                    ...
                    try {
                        ...
                        std::cout <<"set outputmean/std with 0/1, Will result in do nothing when output unstandardization is used ..."\
                            <<std::endl;
                        m_outputMeans  = Cpu::real_vector(m_outputPatternSize, 0.0f);
                        m_outputStdevs = Cpu::real_vector(m_outputPatternSize, 1.0f);
                        m_inputMeans  = Cpu::real_vector(m_inputPatternSize, 0.0f);
                        m_inputStdevs = Cpu::real_vector(m_inputPatternSize, 1.0f);
                    }
                }

				std::cout << "inputMeans & inputStdevs:" << std::endl;
                for(unsigned int i=0; i<m_inputPatternSize; i++){
                    std::cout << m_inputMeans[i] << " ";
                }
                std::cout <<std::endl;
                for(unsigned int i=0; i<m_inputPatternSize; i++){
                    std::cout << m_inputStdevs[i] << " ";
                }
                std::cout <<std::endl;

				std::cout << "outputMeans & outputStdevs:" << std::endl;
                for(unsigned int i=0; i<m_outputPatternSize; i++){
                    std::cout << m_outputMeans[i] << " ";
                }
                std::cout <<std::endl;
                for(unsigned int i=0; i<m_outputPatternSize; i++){
                    std::cout << m_outputStdevs[i] << " ";
                }
                std::cout <<std::endl;
    ...
    Cpu::real_vector DataSet::inputMeans() const
    {
        return m_inputMeans;
    }

    Cpu::real_vector DataSet::inputStdevs() const
    {
