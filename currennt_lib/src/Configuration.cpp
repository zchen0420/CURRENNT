...
Configuration::Configuration(int argc, const char *argv[])
{
    ...
    trainingOptions.add_options()
        ...
        ("rho",                 po::value(&m_rho)              ->default_value((real_t)0.9, "0.9"),    "sets the rho for the the adaptive delta optimizer")
        ("beta1",               po::value(&m_beta1)            ->default_value((real_t)0.9, "0.9"),    "sets the beta1 for the the adaptive moment optimizer")
    ...
    dataFilesOptions.add_options()
        ...
        ("input_include_multiplyer", po::value(&m_includingMultiplers)->default_value(false), "option if a many to multi-many task is included in .nc data")
        ;
    ...
    else if (optimizerString == "adagrad")
        m_optimizer = OPTIMIZER_ADAGRAD;
    else if (optimizerString == "adam")
        m_optimizer = OPTIMIZER_ADAM;
    else if (optimizerString == "adadelta")
        m_optimizer = OPTIMIZER_ADADELTA;
    else {
        std::cout << "ERROR: Invalid optimizer. Possible values: steepest_descent, adagrad, adadelta, adam, rprop." << std::endl;
        exit(1);
    }
    ...
    else if (feedForwardFormatString == "cmp")
        m_feedForwardFormat = FORMAT_CMP;
    else {
...
real_t Configuration::beta1() const
{
    return m_beta1;
}

real_t Configuration::beta2() const
{
    return m_beta2;
}

real_t Configuration::rho() const
{
    return m_rho;
}

real_t Configuration::epsilon() const
{
    return m_epsilon;
}
...
bool Configuration::includingMultiplyers() const
{
    return m_includingMultiplers;
}
