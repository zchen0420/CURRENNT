...
#include "../../currennt_lib/src/optimizers/AdaGradOptimizer.hpp"
#include "../../currennt_lib/src/optimizers/AdaDeltaOptimizer.hpp"
...
void saveNetwork(const NeuralNetwork<TDevice> &nn, const Cpu::real_vector &inputmeans,
const Cpu::real_vector &inputstd, const Cpu::real_vector &outputmeans, const Cpu::real_vector &outputstd,
...
int trainerMain(const Configuration &config)
{
    try {
        ...
        if (config.trainingMode()) {
            ...
            optimizers::Optimizer<TDevice> *opr;


            Cpu::real_vector inputMeans  = trainingSet->inputMeans();
            Cpu::real_vector inputStdevs = trainingSet->inputStdevs();
            Cpu::real_vector outputMeans  = trainingSet->outputMeans();
            Cpu::real_vector outputStdevs = trainingSet->outputStdevs();


            switch (config.optimizer()) {
            case Configuration::OPTIMIZER_STEEPESTDESCENT:
                opr = new optimizers::SteepestDescentOptimizer<TDevice>(
                ...
                optimizer.reset(opr);
                break;
            case Configuration::OPTIMIZER_ADAGRAD:
                opr = new optimizers::AdaGradOptimizer<TDevice>(
                    neuralNetwork, *trainingSet, *validationSet, *testSet,
                    config.maxEpochs(), config.maxEpochsNoBest(), config.validateEvery(), config.testEvery(),
                    config.learningRate(), config.epsilon()
                    );
                optimizer.reset(opr);
                break;
            case Configuration::OPTIMIZER_ADAM:
                opr = new optimizers::AdamOptimizer<TDevice>(
                    neuralNetwork, *trainingSet, *validationSet, *testSet,
                    config.maxEpochs(), config.maxEpochsNoBest(), config.validateEvery(), config.testEvery(),
                ...
                optimizer.reset(opr);
                break;
            case Configuration::OPTIMIZER_ADADELTA:
                opr = new optimizers::AdaDeltaOptimizer<TDevice>(
                    neuralNetwork, *trainingSet, *validationSet, *testSet,
                    config.maxEpochs(), config.maxEpochsNoBest(), config.validateEvery(), config.testEvery(),
                    config.rho(), config.epsilon()
                    );
                optimizer.reset(opr);
                break;
            default:
                ...
                if (!validationSet->empty() && optimizer->currentEpoch() % config.validateEvery() == 0) {
                    if (optimizer->epochsSinceLowestValidationError() == 0) {
                        ...
                        if (config.autosaveBest()) {
                            ...
                            saveFileS << ".best.jsn";
                            saveNetwork(neuralNetwork, inputMeans, inputStdevs, outputMeans, outputStdevs, saveFileS.str());
                        }
                    }
                    else
            ...
            saveNetwork(neuralNetwork, inputMeans, inputStdevs, outputMeans, outputStdevs, config.trainedNetworkFile());
            printf("done.\n");
        ...
        // evaluation mode
            ...
            else if (config.feedForwardFormat() == Configuration::FORMAT_CMP) {
                // process all data set fractions
                int fracIdx = 0;
                boost::shared_ptr<data_sets::DataSetFraction> frac;
                while (((frac = feedForwardSet->getNextFraction()))) {
                    ...
                    for (int psIdx = 0; psIdx < (int)outputs.size(); ++psIdx) {
                        if (outputs[psIdx].size() > 0) {
                            ...
                            //seqTag += ".htk";
                            //std::cout << seqTag << std::endl;
                            boost::filesystem::path seqPath(frac->seqInfo(psIdx).seqTag + ".cmp");
                            std::string filename(seqPath.filename().string());
                            boost::filesystem::path oPath = boost::filesystem::path(config.feedForwardOutputFile()) / seqPath.relative_path().parent_path();
                            boost::filesystem::create_directories(oPath);
                            boost::filesystem::path filepath = oPath / filename;
...
int main(int argc, const char *argv[])
{
    ...
    //print configuration
    std::cout<<config.serializedOptions()<<std::endl<<std::endl;
...
void printLayers(const NeuralNetwork<TDevice> &nn)
{
    ...
    for (int i = 0; i < (int)nn.layers().size(); ++i) {
        ...
        if (tl) {
            ...
            if (dynamic_cast<const layers::LstmLayer<TDevice>*>(tl))
                printf(", time lag: %d", static_cast<const layers::LstmLayer<TDevice>*>(tl)->ouputLag());
...
void printOptimizer(const Configuration &config, const optimizers::Optimizer<TDevice> &optimizer)
{
    if (dynamic_cast<const optimizers::SteepestDescentOptimizer<TDevice>*>(&optimizer)) {
        ...
        printf("\n");
    } else if (dynamic_cast<const optimizers::AdaGradOptimizer<TDevice>*>(&optimizer)) {
        printf("Optimizer type:            AdaGrad\n");
        printf("Learning rate:             %g\n", (double)config.learningRate());
        printf("Epsilon:                   %g\n", (double)config.epsilon());
        printf("\n");
        ...
        printf("Epsilon:                   %g\n", (double)config.epsilon());
        printf("\n");
    } else if (dynamic_cast<const optimizers::AdaDeltaOptimizer<TDevice>*>(&optimizer)) {
        printf("Optimizer type:            AdaDelta\n");
        printf("Rho:                       %g\n", (double)config.rho());
        printf("Epsilon:                   %g\n", (double)config.epsilon());
        printf("\n");
    }
...
void exportArray(const char* vecptr, const Cpu::real_vector & vec, const helpers::JsonValue &weightsObject, const helpers::JsonAllocator &allocator)
{

    if (!weightsObject->IsObject())
        throw std::runtime_error("The JSON value is not an object");

    rapidjson::Value outputmeansArray(rapidjson::kArrayType);
    int count = vec.size();
    outputmeansArray.Reserve(count, allocator);
    for (int i = 0; i < count; ++i)
        outputmeansArray.PushBack(vec[i], allocator);
    weightsObject->AddMember(vecptr,    outputmeansArray,    allocator);

}

void exportNormData(const Cpu::real_vector &inputmeans, const Cpu::real_vector &inputstd,
       const Cpu::real_vector &outputmeans, const Cpu::real_vector &outputstd, const helpers::JsonDocument &jsonDoc)

{
    ...
    exportArray("inputMeans", inputmeans, &weightsObject, &jsonDoc->GetAllocator());
    exportArray("inputStdevs", inputstd, &weightsObject, &jsonDoc->GetAllocator());
    exportArray("outputMeans", outputmeans, &weightsObject, &jsonDoc->GetAllocator());
    exportArray("outputStdevs", outputstd, &weightsObject, &jsonDoc->GetAllocator());


    // if the section already exists, we delete it first
    if (jsonDoc->HasMember("normdata"))
        jsonDoc->RemoveMember("normdata");

    // add the section to the JSON document
    jsonDoc->AddMember("normdata", weightsObject, jsonDoc->GetAllocator());
}
        ...
        const std::string &filename)
{
    ...
    exportNormData(inputmeans, inputstd, outputmeans, outputstd, &jsonDoc);
